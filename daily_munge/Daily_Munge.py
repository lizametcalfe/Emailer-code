...
#Changes made by Ben Clapperton
...
def  daily_munge(supermarket, date):  
	...
	def new_match():	
		includes = [i.strip() for i in supermarket2.ix[row_index,'include'].split(' ') if i.strip()]
        excludes = [e.strip() for e in supermarket2.ix[row_index,'exclude'].split(' ') if e.strip()]
        removes = [r.strip() for r in supermarket2.ix[row_index,'remove'].split(' ') if r.strip()]
    ...
    def category1():	
        """ Function to set the sub_index for the product, based
            ...
            RB note there is no category1 in the dictionary"""
          
        count = 0
        
        for s in (supermarket2.ix[row_index, 'category1'].split()):
    ...
    def multiples():	
        ...
        if any(s in supermarket for s in('tesco', 'waitrose')) and 'TEA' in supermarket2.ix[row_index, 'ons_item_name'].strip().upper():
	...
	if there is no relevant volume information"""
        ...
        search =  supermarket2.ix[row_index,'std_unit'].strip()
        ...
        if VOL ==-1 or ('TEA' in name): 
            ...
            return int(VOL)*int(supermarket2.ix[row_index, 'num_units']) 
    ...
    def extract_AVB():
        ...
        if any(s in  (supermarket2.ix[row_index, 'ons_item_name'].strip().upper()) for s in ("BITTER" , "LAGER", "CIDER", "WINE")):
            startpos = name.find("(")
            stoppos = name.find("%")
    ...
    '''
    # NEED TO LOCALISE TO THE OUTPUT HERE: RB
    ...
    if counter > 0: 
        ...
        '''
        # NEED TO LOCALISE THE DICTIONARY
        # /home/projectt004/Prices/KPIs/match_dictionary.csv
        ...
        for row_index, row in supermarket2.iterrows():
            ...
            try:           
                supermarket2.ix[row_index,'new_match'] = new_match()
            ...
            # derive multiples
            try:  
                supermarket2.ix[row_index,'num_units'] = multiples()
            ...
            try:    
                supermarket2.ix[row_index,'volume_total']=create_vol()
            ...
            try:     
                supermarket2.ix[row_index, 'sub_type'] = category1()
            ...
            # Define brand type
            try:    
                supermarket2.ix[row_index, 'brand'] = brands()
            ...
            # Define range type
            try:       
                supermarket2.ix[row_index, 'range'] = range_type()    
            ...
            try:    
                supermarket2.ix[row_index, 'alcohol_volume'] = extract_AVB()
        ...
        '''
        # NOT SURE WHY THERE ARE SO MANY DIFFERENT LOCATIONS 
        # /home/projectt004/Prices/Data/
        ...
        try:
            ...
            L = len(oldfile)
        # /nas/projects/ProjectT004/Data/ BELOW
            
            if L > 0: 
            	print "L equals ", L
                
                #Make a back up of today's file to NAS before appending new data
                oldfile.to_csv('' + supermarket + '/home/mint/longditudal/output/_longitudinal_backup.csv', index = False)
...
daily_munge(supermarket='waitrose', date=dates)
